...
<head>
    ...
    <link href="{{ asset('frontend_assets/css/post.css') }}" rel="stylesheet" />
...
$(document).ready(function() {        
    $("form").submit(function(e){
		...
		var name = $.trim($("#ontacts5-5-name").val());
		var email = $.trim($("#contacts5-5-email").val());
		var message = $.trim($("#contacts5-5-message").val());
			...
			$.post("/processMail",
                    ...
                    function(data){
                        if(data == 'success'){
                            //depopulate form
                            $("#ontacts5-5-name").val('');
                            $("#contacts5-5-email").val('');
                            $("#contacts5-5-message").val('');
                            
                            $('#status').html("<font style='color:red;'>* Thank you for contacting us.</font>");
                        }
                        else{
                            $('#status').html("<font style='color:red;'>* Mail not sent, pls try after some time.</font>");
                        }
                    });	
		}
    });
